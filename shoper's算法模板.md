...
void solve() {
    ...
    vector<Point<ff>>R, L;
    for (int i = 1; i <= n; i++) {
    	while (R.size() > 1 && sgn((R.back() - R[R.size() - 2]) ^ (a[i] - R.back())) < 0) {
    		R.pop_back();
    	}
    	R.push_back(a[i]);
    }
    for (int i = n; i >= 1; i--) {
    	while (L.size() > 1 && sgn((L.back() - L[L.size() - 2]) ^ (a[i] - L.back())) < 0) {
    		L.pop_back();
    	}
    	L.push_back(a[i]);
    }
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
    	while (1) {
    		auto v = a[i % n + 1] - a[i];
    		auto u = a[j % n + 1] - a[j];
    		if (sgn(v ^ u) > 0 || (sgn(v ^ u) == 0 && (u * v) > 0)) {
    			j = j % n + 1;
    		}
    		else break;
    	}
    	ans = max({ans, (a[i] - a[j]).length(), (a[i % n + 1] - a[j]).length()});
    }
...
void solve() {
    ...
    while (q--) {
    	...
    	Line<ff> line = Line<ff>::make(p1, p2);
    	vector<Point<ff>>tmp;
    	for (int i = 1; i <= n; i++) {
    		int d1 = line.v ^ (a[i] - line.p);
    		int d2 = line.v ^ (a[i % n + 1] - line.p);
    		if (d1 >= 0) {
    			tmp.push_back(a[i]);
    		}
    		if (d1 * d2 < 0) {
    			tmp.push_back(inter(line, Line<ff>::make(a[i % n + 1], a[i])));
    		}
    	}
    	cout << Area(tmp) << '\n';
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
    	...
    	Point<ff> lo = {-INF, a[i].y - d, -1};
    	auto it = s.lower_bound(lo);
    	while (it != s.end() && it -> y <= a[i].y + d) {
    		d = min(d, (a[i] - *it).length());
    		it++;
    	}
    	s.insert(a[i]);
    }
...
- `a + b = (a | b) + (a & b)`  
  因为 a|b = a^b + a&b，代入上式就得

- `a ^ b = (a + b) - 2*(a & b)`  
  从原公式移项

- `a | b = (a ^ b) + (a & b)`  
  OR 拆成 XOR + AND，两两互斥的部分和重叠部分的直接加和

- `a & b = (a + b) - (a | b)`  
  容斥原理，AND 就是两者之和减去 OR 覆盖的
